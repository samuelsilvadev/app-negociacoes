...
export class NegociacaoService {
	...
	obterNegociacoes() {
		return Promise.all([
			...
			.then(negociacoes =>
				negociacoes.reduce(
					(arraymenor, array) => arraymenor.concat(array),
					[]
				)
			)
			.catch(err => {
	...
	lista() {
		return ConnectionFactory.getConnection()
			...
			.catch(e => {
				throw new Error(
					'Não foi possível listar as negociações' + e.target.name
				);
			});
	}
